        ...
        if($user != null){
            foreach ($user->applicationResponses as $applicationRespons){
                array_push($registrations,[
                    ...
                    'actions' => '
                        <a href="' . url('/forms/users/'. $user->id . '/detail/'. $applicationRespons->agenda_id). '"><span title="' . get("inschrijven.applicationFormDetail") . '" class="ion-eye" aria-hidden="true"></span></a>
